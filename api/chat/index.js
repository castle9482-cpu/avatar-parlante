export default async function handler(req, res) {
  ...
  const allowed = origin === 'https://castle9482-cpu.github.io' || origin.endsWith('.vercel.app');
  ...
  try {
    ...
    if (tavilyKey && needsWeb) {
      try {
        const tr = await fetch('https://api.tavily.com/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ api_key: tavilyKey, query: latest, search_depth: 'basic', max_results: 5, include_answer: true }) });
        if (tr.ok) {
